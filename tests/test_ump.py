...
TEST_PACKETS = [
    ...
    pytest.param(
        ...
        ump.MIDI2NoteOn(group=2, channel=4, note=64, velocity=2500, attribute_type=3, attribute_data=0x1234),
        id="midi2-note-on",
    ),
    pytest.param(
        [0x42844003, 0x09c41234],
        ump.MIDI2NoteOff(group=2, channel=4, note=64, velocity=2500, attribute_type=3, attribute_data=0x1234),
        id="midi2-note-off",
    ),
    pytest.param(
    ...
    pytest.param(
        ...
        ump.MIDI2ProgramChange(group=2, channel=4, program=42, bank_valid=True, bank=2500),
    ...
    pytest.param(
        ...
        ump.Data128(group=2, status=ump.UMPStreamFormat.COMPLETE, stream_id=42, data=[1, 2, 3, 4, 5, 6, 7, 8]),
        id="data128",
    ),

    # UMP Stream Messages
    pytest.param(
        [0xf0000101, 0x0000001f, 0x00000000, 0x00000000],
        ump.EndpointDiscovery(form=0, ump_version_major=1, ump_version_minor=1, filter=0x1f),
        id="endpoint-discovery",
    ),
    pytest.param(
        [0xf0010101, 0x91000303, 0x00000000, 0x00000000],
        ump.EndpointInfoNotification(form=0, ump_version_major=1, ump_version_minor=1, static=True, n_function_blocks=17, midi1=True, midi2=True, rxjr=True, txjr=True),
        id="endpoint-info-notification"
    ),
    pytest.param(
        [0xf0020000, 0x00000007, 0x01000200, 0x01020304],
        ump.DeviceIdentityNotification(form=0, device_manufacturer=(0, 0, 7), device_family=1, device_family_model=2, software_revision=(1, 2, 3, 4)),
        id="device-identity-notification",
    ),
    pytest.param(
        [0xf003e282, 0xac75726f, 0x7261636b, 0x00000000],
        ump.EndpointNameNotification(form=0, name="€urorack"),
        id="endpoint-name-notification",
    ),
    pytest.param(
        [0xf0044177, 0x65736f6d, 0x65207072, 0x6f640000],
        ump.ProductInstanceIdNotification(form=0, product_instance_id="Awesome prod"),
        id="product-instance-id-notification",
    ),
    pytest.param(
        [0xf0050180, 0x00000000, 0x00000000, 0x00000000],
        ump.StreamConfigurationRequest(form=0, protocol=1, extensions=True, reserved=0),
        id="stream-configuration-request",
    ),
    pytest.param(
        [0xf0060180, 0x00000000, 0x00000000, 0x00000000],
        ump.StreamConfigurationNotification(form=0, protocol=1, extensions=True, reserved=0),
        id="stream-configuration-notification",
    ),
    pytest.param(
        [0xf0100100, 0x00000000, 0x00000000, 0x00000000],
        ump.FunctionBlockDiscovery(form=0, function_block_filter=1, reserved=0),
        id="function-block-discovery",
    ),
    pytest.param(
        [0xf0118133, 0x01020004, 0x00000000, 0x00000000],
        ump.FunctionBlockInfoNotification(form=0, active=1, function_block_id=1, ui_hint_output=True, ui_hint_input=True, midi1=0, is_input=True, is_output=True, first_group=1, number_of_groups=2, midi_ci_version=0, max_sysex_8_streams=4),
        id="function-block-info-notification",
    ),
    pytest.param(
        [0xf0120148, 0x656c6c6f, 0x00000000, 0x00000000],
        ump.FunctionBlockNameNotification(form=0, function_block_id=1, name="Hello"),
        id="function-block-name-notification",
    ),
    pytest.param(
        [0xf0200000, 0x00000000, 0x00000000, 0x00000000],
        ump.StartOfClip(form=0, reserved=0),
        id="start-of-clip",
    ),
    pytest.param(
        [0xf0210000, 0x00000000, 0x00000000, 0x00000000],
        ump.EndOfClip(form=0, reserved=0),
        id="end-of-clip",
    ),
]
